...
with st.sidebar:
    ...
    st.divider()
    st.info("💡 **Tips Prompt:** Gunakan menu 'Upload Materi' agar soal lebih presisi sesuai buku pegangan siswa.")

st.title("🇮🇩 Nusantara AI: Generator Soal AKM")
st.markdown("Buat soal berkualitas standar Kurikulum Merdeka dengan Konteks Indonesia.")

# --- TAB MENU ---
tab1, tab2 = st.tabs(["📝 Generator Soal", "📂 Upload Materi"])
...
with tab1:
    ...
    if st.button("🚀 GENERATE SOAL SEKARANG", type="primary"):
        if not api_key:
            ...
            try:
                ...
                for idx, item in enumerate(data_soal['soal_list']):
                    with st.container(border=True):
                        ...
                        if tipe_vis == "CODING":
                            st.caption("🤖 Membuat Grafik Presisi (Python Matplotlib)...")
                ...
                with col_dl2:
                    st.button("🔄 Buat Ulang", type="secondary", use_container_width=True)
